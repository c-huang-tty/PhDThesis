...
def xgboost_shap_C(pattern):
    ...
    if pattern == 0:
        ...
        plt.savefig('../Manuscript/Figures/' + 'importance-bar_C.pdf', bbox_inches = "tight")
        plt.show()
        ...
        for label in labels:
            # label.set_fontname('Times New Roman')
            label.set_fontsize(label_font_size)
        plt.savefig('../Manuscript/Figures/' + 'importance-beeswarm_C.pdf', bbox_inches = "tight")
        plt.show()
    ...
    if pattern == 1:
        for i in range(10):
            ...
            plt.savefig('../Manuscript/Figures/' + 'MainEffects_C_' + fileName[i] + '.pdf', bbox_inches = "tight")
            plt.show()
    ...
    if pattern == 2:
        for i in range(10):
            ...
            plt.savefig('../Manuscript/Figures/' + 'InteractionEffects_C_' + fileName[i] + '.pdf', bbox_inches = "tight")
            plt.show()
    ...
    if pattern == 3:
        for i in range(2):
            ...
            for label in labels:
                # label.set_fontname('Times New Roman')
                label.set_fontsize(label_font_size)
            plt.savefig('../Manuscript/Figures/' + 'LocalExplanation_C_' + fileNameLocal[i] + '.pdf', bbox_inches = "tight")
            plt.show()
...
def model_train_test_kNN_C():
    ...
    cv = RepeatedKFold(n_splits = 10, n_repeats = 5, random_state = 1)
    # define model evaluation method
    parameters = {'n_neighbors': [3, 4, 5, 6, 7, 8, 9], #so called `eta` value
    ...
    model_grid.fit(x_train_onehot, y_train)
    
    print('Best Score: ', model_grid.best_score_)
    print('Best Parameters: ', model_grid.best_params_)
    
    y_pred = model_grid.predict(x_test_onehot)
...
def model_train_test_SVM_C(kernel):
    '''
    ...
    cv = RepeatedKFold(n_splits = 10, n_repeats = 5, random_state = 1)
    # define model evaluation method
    parameters = {'kernel': [kernel], #so called `eta` value
                  'degree': [2, 3, 4, 5],
                  'C': [1, 10, 100]
                  # 'min_child_weight': [4],
                  # 'silent': [1],
...
# model_train_test_SVM_C('linear')
# model_train_test_SVM_C('poly')
# model_train_test_SVM_C('rbf')
...
def model_train_test_DT_C():
    ...
    cv = RepeatedKFold(n_splits = 10, n_repeats = 5, random_state = 1)
    # define model evaluation method
    parameters = {'criterion': ['gini', 'entropy'], #so called `eta` value
