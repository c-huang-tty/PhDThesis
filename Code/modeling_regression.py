...
def xgboost_shap(pattern):
    ...
    if pattern == 0:
        ...
        plt.savefig('../Manuscript/Figures/' + 'importance-bar.pdf', bbox_inches = "tight")
        plt.show()
        ...
        for label in labels:
            # label.set_fontname('Times New Roman')
            label.set_fontsize(label_font_size)
        plt.savefig('../Manuscript/Figures/' + 'importance-beeswarm.pdf', bbox_inches = "tight")
        plt.show()
    ...
    if pattern == 1:
        for i in range(8):
            ...
            plt.savefig('../Manuscript/Figures/' + 'MainEffects_' + fileName[i] + '.pdf', bbox_inches = "tight")
            plt.show()
    ...
    if pattern == 2:
        for i in range(8):
            ...
            plt.savefig('../Manuscript/Figures/' + 'InteractionEffects_' + fileName[i] + '.pdf', bbox_inches = "tight")
            plt.show()
    ...
    if pattern == 3:
        for i in range(3):
            ...
            for label in labels:
                # label.set_fontname('Times New Roman')
                label.set_fontsize(label_font_size)
            plt.savefig('../Manuscript/Figures/' + 'LocalExplanation_' + fileNameLocal[i] + '.pdf', bbox_inches = "tight")
            plt.show()
...
def model_train_test_kNN():
    ...
    # define model evaluation method
    parameters = {'n_neighbors': [3, 5, 6, 7, 8, 9], #so called `eta` value
...
def model_train_test_SVM(kernel):
    '''
...
model_train_test_SVM('linear')
model_train_test_SVM('poly')
model_train_test_SVM('rbf')
...
def model_train_test_DT():
    ...
    # define model evaluation method
    parameters = {'criterion': ['squared_error', 'absolute_error'], #so called `eta` value
